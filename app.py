...
def predict():
    try:
        ...
        # Compléter les features manquantes avec leur médiane
        complete_data = {feature: data.get(feature, feature_medians.get(feature, 0)) for feature in expected_features}
        ...
        # Faire la prédiction
        seuil = 0.35  # Seuil optimal trouvé avant
        prediction = 1 if prediction_proba[1] > seuil else 0

        return jsonify({"prediction": int(prediction), "probability": float(prediction_proba[1])})
